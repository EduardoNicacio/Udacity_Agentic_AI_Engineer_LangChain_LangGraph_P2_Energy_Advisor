...
1. Run notebook `01_db_setup.ipynb` to initialize the database and populate it with sample energy usage and solar generation data.
2. Run notebook `02_rag_setup.ipynb` to set up the RAG pipeline with energy-saving tips and best practices.
3. Expand the knowledge base by adding **at least 5 additional energy-saving documents** to the data/documents/ folder. Make sure you have diverse topics covering:
...
1. Review the existing tools in `tools.py` to understand the available capabilities.
2. Enhance the agent in `agent.py` by:
...
3. Test and evaluate your agent using the scenarios in `03_run_and_evaluate.ipynb`.
...
You're receiving the starter code, but please submit your project with all artifacts under `ecohome_solution/` or the project's root folder. We'll not look into `ecohome_starter/`. Make sure you are copying and pasting the code from `ecohome_starter/` to `ecohome_solution/` before modifying it.
